...
def get_all_book(detailed: bool = False, database: Session = Depends(get_db)) -> list:
    ...
    if detailed:
        ...
        for book_item in book_list:
            ...
            author_list = database.query(Author).join(author_book, Author.id == author_book.c.writtenBy).filter(author_book.c.publishes == book_item.id).all()
            item_dict['writtenBy'] = []
...
async def create_book(book_data: BookCreate, database: Session = Depends(get_db)) -> Book:
    ...
    db_book = Book(
        release=book_data.release,        title=book_data.title,        pages=book_data.pages,        locatedIn_id=book_data.locatedIn        )
...
async def bulk_create_book(items: list[BookCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_book = Book(
                release=item_data.release,                title=item_data.title,                pages=item_data.pages,                locatedIn_id=item_data.locatedIn            )
            database.add(db_book)
...
def get_all_author(detailed: bool = False, database: Session = Depends(get_db)) -> list:
    ...
    if detailed:
        ...
        for author_item in author_list:
            ...
            # Add many-to-many and one-to-many relationship objects (full details)
            book_list = database.query(Book).join(author_book, Book.id == author_book.c.publishes).filter(author_book.c.writtenBy == author_item.id).all()
            item_dict['publishes'] = []
...
async def create_library(library_data: LibraryCreate, database: Session = Depends(get_db)) -> Library:
    ...
    db_library = Library(
        name=library_data.name,        address=library_data.address        )
...
async def bulk_create_library(items: list[LibraryCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_library = Library(
                name=item_data.name,                address=item_data.address            )
            database.add(db_library)
            database.flush()  # Get ID without committing
